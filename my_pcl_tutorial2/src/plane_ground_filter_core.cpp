...
PlaneGroundFilter::PlaneGroundFilter(ros::NodeHandle &nh)
{
    ...
    //sub_point_cloud_ = nh.subscribe("/velodyne_points_voxelized", 10, &PlaneGroundFilter::point_cb, this); //input lidar point
    //sub_point_cloud_ = nh.subscribe("/velodyne_points_roied", 10, &PlaneGroundFilter::point_cb, this);
    sub_steer = nh.subscribe("/output", 10, &PlaneGroundFilter::steer_deg, this);
    ...
    nh.getParam("min_distance_x", min_distance_x_);
    ROS_INFO("min_distance_x: %f", min_distance_x_);
    nh.getParam("max_distance_x", max_distance_x_);
    ROS_INFO("max_distance_x: %f", max_distance_x_);
    nh.getParam("min_distance_y", min_distance_y_);
    ROS_INFO("min_distance_y: %f", min_distance_y_);
    nh.getParam("max_distance_y", max_distance_y_);
    ROS_INFO("max_distance_y: %f", max_distance_y_);
    ...
    ROS_INFO("voxel_size: %f", voxel_size_);
    nh.getParam("voxel_key", voxel_key_);
    ROS_INFO("voxel_key: %d", voxel_key_);
...
void PlaneGroundFilter::steer_deg(const erp42_communication::ERP42_feedback::ConstPtr& msg)
{
    steer = msg->steer_degree;
    //std::cout << "steer_degree : " << msg->steer_degree << "\t" << steer << std::endl;
}

void PlaneGroundFilter::rectangular_roi(const pcl::PointCloud<VPoint>::Ptr in,
                                        const pcl::PointCloud<VPoint>::Ptr out) // crop points higher than clip height
{
    pcl::PointCloud<VPoint>::Ptr region1 (new pcl::PointCloud<VPoint>);
    pcl::PointCloud<VPoint>::Ptr region2 (new pcl::PointCloud<VPoint>);
#pragma omp for
    for (size_t i = 0; i < in->points.size(); i++)
    {
        if (in->points[i].y > max_distance_y_ || in->points[i].y < -max_distance_y_ || in->points[i].x > max_distance_x_ || in->points[i].x < -max_distance_x_)
        {
            continue;
        }
        if ( (-min_distance_ < in->points[i].y && in->points[i].y  < min_distance_) && (-min_distance_ < in->points[i].x && in->points[i].x < min_distance_))
            continue;
        if (in->points[i].y< min_distance_y_ && in->points[i].y> -min_distance_y_)
        {
            region1->push_back(in->points[i]);
        }
        else
        {
            region2->push_back(in->points[i]);
        }
    }
    
    if(voxel_key_ == 1){
        voxelize(region2,out);
        (*out)+=(*region1);
        ROS_INFO_STREAM("origin: "<<in->points.size()<<" After: "<<out->points.size()<<" voxel_size: "<< voxel_size_);
    }
    if(voxel_key_== 0 ){
        (*out)+=(*region1);
        (*out)+=(*region2);
        ROS_INFO_STREAM("voxel_off");
    }
}
...
*/
void PlaneGroundFilter::estimate_plane_(void) // estimate if it is plane or not 
{
    // Create covarian matrix in single pass.
    // TODO: compare the efficiency.
    Eigen::Matrix3f cov;
    Eigen::Vector4f pc_mean;
    pcl::computeMeanAndCovarianceMatrix(*g_ground_pc, cov, pc_mean); // cloud, covariance_matrix, centroid / #include <pcl/common/centroid.h>
...
void PlaneGroundFilter::voxelize(const pcl::PointCloud<VPoint>::Ptr in, const pcl::PointCloud<VPoint>::Ptr out)
{
    pcl::VoxelGrid<VPoint> voxel_filter;
    float voxel_size = voxel_size_; // smaller -> more points, larger -> less points
...
void PlaneGroundFilter::post_process(const pcl::PointCloud<VPoint>::Ptr in, const pcl::PointCloud<VPoint>::Ptr out)
{
    ...
    /*if (str_steer) // when str_steer != 0
    {
        if (steer > 3 || steer < -3) // out of 3 degree
            num_steer++;
        else num_steer=0;

        if (num_steer>=8) { // corner
            str_steer=0; // cylinder
            num_steer=16;
        }
    }
    else {
        ...
        if (!num_steer) // only num_steer=0 -> true else false
            str_steer=1; // straight
    }
    if (str_steer==1) {
        rectangular_roi(cliped_pc_ptr, out);
        std::cout << "Rectangular_roi" << std::endl;
    }
    else {
        remove_close_far_pt(cliped_pc_ptr, out);
        std::cout << "Cylinder_roi" << std::endl;
    }
    */
   rectangular_roi(cliped_pc_ptr, out);
}

void PlaneGroundFilter::point_cb(const sensor_msgs::PointCloud2ConstPtr &in_cloud_ptr) // send msg
{
    // 1.Msg to pointcloud
