...
private:
  ...
  int str_steer=1;
  int sensor_model_;
  double sensor_height_, clip_height_, min_distance_x_, max_distance_x_, min_distance_y_, max_distance_y_, min_distance_, max_distance_;
  int num_seg_ = 1;
  ...
  float d_, th_dist_d_, voxel_size_;
  int voxel_key_;
  MatrixXf normal_;
  ...
  void steer_deg(const erp42_communication::ERP42_feedback::ConstPtr& msg);
  void rectangular_roi(const pcl::PointCloud<VPoint>::Ptr in, const pcl::PointCloud<VPoint>::Ptr out);
  ...
  void remove_close_far_pt(const pcl::PointCloud<VPoint>::Ptr in,
                       const pcl::PointCloud<VPoint>::Ptr out);
  void voxelize(const pcl::PointCloud<VPoint>::Ptr in,
                       const pcl::PointCloud<VPoint>::Ptr out);
public:
